...
def train_lstm_model(model, criterion, optimizer, dataloaders, num_epochs=10):
    since = time.time()
...
def train_transformer_model(model, criterion, optimizer, dataloaders, num_epochs=10):
    since = time.time()
    train_batches = len(dataloaders["train"])
    val_batches = len(dataloaders["val"])
    metrics = {
